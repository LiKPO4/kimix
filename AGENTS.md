...
- 仓库大量源文件是 CRLF 或混合行尾（如 `src/App.tsx`、`Composer.tsx`），Read 会把 CR 显示为 `\r`；Edit 单行替换通常可行，跨行 `old_string` 常因行尾不一致匹配失败，不要反复重试。同一文件多点位编辑优先用 python 脚本一次完成：`open(..., newline=)` 读取、逐条 `assert content.count(old) == 1` 再替换、新插入行行尾跟随局部。
- 诊断/探测产生的 `.tmp-*` 文件用完即删；提交前 `git status` 发现这类遗留时提醒用户清理，不擅自删除。
...
2. **改完后的标准验证流程**：
   ...
   - 版本号同步 bump（只改 `package.json`，其余自动跟随）。
   - 不要求 agent 自行截图验收；由用户截图验收并回传。版本号对上后，agent 再根据用户截图反馈判断内容问题。

3. **默认 shell 的 PATH 问题**：
   本机默认 PATH 不含 Node/pnpm，执行构建前必须前置：
   - cmd：`set "PATH=C:\Program Files\nodejs;C:\Users\lijialin08\AppData\Roaming\npm;%PATH%"`
   - Git Bash（本 agent 的 Bash 工具）：`export PATH="/c/Program Files/nodejs:/c/Users/lijialin08/AppData/Roaming/npm:$PATH"`
...
每轮有软件本体改动（版本 bump）或排查出稳定根因/已知边界时，收尾提交前同步记一条；纯 UI 微调、纯文档改动可不记。日期倒序，最新在顶部。

### TASK_STATE 归档机制（超阈值必须执行）

- **阈值**：`TASK_STATE.md` 超过约 2000 行时，把早期记录归档到 `TASK_HISTORY.md`，不要拖到数千行。
- **保留窗口**：保留最近约 2 周或当前版本周期的条目（按工作连续性判断，保留区必须完整覆盖进行中工作的上下文）。
- **归档流程**：
  1. 按日期条目（`## YYYY-MM-DD`）整体切分，保留区之外的内容移入 `TASK_HISTORY.md`；文件尾部常有乱序追加块，其中属于保留窗口的条目必须摘回保留区对应日期群。
  2. `TASK_HISTORY.md` 按归档批次追加，每批次头部标注**收纳日期范围与归档时间**；内容仅搬运、不改写。
  3. 归档时顺手补记此前漏记的近期工作（版本改动、根因、已知边界），避免丢交接上下文。
  4. 校验日期条目数守恒（保留 + 归档 = 原有 + 新增），并抽查两文件边界处无被截断的条目。
- 归档属于纯文档改动，不递增版本号。
