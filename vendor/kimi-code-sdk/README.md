...
| Official base | `75ac010bcb2050338444455de8328492d152c919` (tag `@moonshot-ai/kimi-code@0.43.1`) |
...
| Validated against CLI | source tag `@moonshot-ai/kimi-code@0.43.1` |
| Bundled on | 2026-09-15 |
...
Kimi Code `0.43.0`/`0.43.1` keep node-sdk at `0.20.0` but change the bundled
`agent-core-v2` engine substantially (124 files, version stays `0.4.3`):
- AI session titles graduate from the experimental `auto_session_title` flag —
  titles are generated by default after the first turn and broadcast via
  `session.meta.updated`; Kimix already relays official titles through its runtime
  snapshot path.
- `loop_control.compaction_max_attempts` (default 5) bounds total retry attempts
  for a failing compaction request.
- `KIMI_CODE_PERMISSION_MODE_REMINDER=0` stops injecting auto permission-mode
  reminders into the model context.
- MCP: `dynamically_loaded_tools` capability, per-server `deferred` field for
  `select_tools` on-demand loading, `select_tools` registration fix, and MCP
  attachments that cannot be delivered directly are preserved.
- Goal time budgets exclude time spent with the session closed and the 24-hour
  cap is removed; `rm -rf` on `/tmp`/`/temp`-only paths no longer asks; steering
  messages interrupt waits for background tasks; startup warns about `[models]`
  entries missing the `model` field.
- Reliability: LLM retry crash after a streamed partial tool call, Tower
  messaging/worktrees/review-to-merge and roster misattribution fixes, subagent
  scope memory release, per-round swarm render slowdown, and event-loop/GC churn
  with many concurrent subagents.
- `kimi upgrade -y`, Remote Control changes, terminal-UI updates, and the web
  app's UI work are CLI/web-only and not part of this bundle.
