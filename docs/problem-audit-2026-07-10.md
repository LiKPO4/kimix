...
**审计日期**：2026-07-10
**当前版本**：v2.15.0（commit `aeca7c9`）
**审计来源**：代码审查清单 + 关键证据文件抽查 + 2026-07-10 复核闭环
**状态**：复核闭环已完成，见文末“复核闭环”章节
...
- **验证状态**：未抽查，基于审查结论纳入
- **修复状态**：已修复（commit `6bd91af`）
- **说明**：`Composer.tsx` 为排队消息行的“更多”按钮增加下拉菜单，提供“移到最前 / 上移 / 下移”；使用 store 已有 `promotePendingMessage` 与 `movePendingMessage`，首尾项自动禁用，菜单可键盘操作，点击外部关闭。
...
- **修复状态**：已修复（commit `8df5be5`）
- **说明**：`electron/main.ts` 新增 `validateZipArchive`，按条目数、单条目解压后大小、解压后总体积三重校验；Skill 导入限制 500 条目/单条 20MB/总计 100MB，会话快照 ZIP 限制 2000 条目/单条 50MB/总计 200MB，超限直接抛错。
...
- **验证状态**：未抽查，基于审查结论纳入
- **修复状态**：已修复（commit `2a20294`）
- **说明**：`MarkdownRenderer` 新增 `collapsibleThreshold`（默认 50,000 字符）。超过阈值时，初始按段落/行边界截断到阈值附近，仅解析并渲染前半部分；底部显示“点击展开剩余 N 字符”按钮，用户主动点击后才渲染完整内容。
...
- **验证状态**：未抽查，基于审查结论纳入
- **修复状态**：已修复（commit `6eb647a`）
- **说明**：新增设置项 `notificationShowContent`，默认 `false`。渲染进程和主进程都读取该设置：关闭时通知正文使用通用提示（“当前轮次处理已完成……”），不再展示 Agent 回复或澄清问题的摘要；设置面板“完成通知”中增加“通知中显示对话内容”开关。
...
本轮针对审计结论中“未真正闭环或引入回归”的 14 项问题进行了修复与测试补充，代码已合并至 `master`（HEAD `aeca7c9`，版本 `v2.15.0`）。

### 已完成的修复

| 问题 | 说明 | 关键提交 |
|------|------|----------|
| 5 | localStorage 会话持久化迁移至 IndexedDB，图片引用分离，增加失败 toast 与串行化队列 | `029b1e7` |
| 1 | 撤销文件增加工作区边界校验、回收站处理，并在撤销前比较内容哈希，冲突时二次确认 | `5f7bcf0` |
| 7 | `@文件` 本地回退搜索改为按扫描条目数让出事件循环 | `719a54d` |
| 22 | 弹窗 Hook 增加焦点陷阱（Tab/Shfit+Tab 循环、容器 `tabIndex=-1`） | `2ff2479` |
| 23/24 | Escape 停止当前任务改用 `isSessionRuntimeRunning` 判断；全局快捷键在 Composer 输入焦点下仍可触发（缩放等编辑类快捷键除外） | `dbab7c8` |
| 35 | Hook matcher 增加嵌套量词与反向引用检测，命中风险模式时回退为子串匹配 | `8f89977` |
| 10/13 | macOS 更新仅接受匹配架构或 universal 包；下载更新包缺少 SHA256 时拒绝自动安装并引导至 Release 页面 | `7071df8` |
| 28 | 同名技能启用时按名称去重，后出现的覆盖先出现的，避免同一路径被覆盖 | `f1ad7e5` |
| 31 | 会话/技能 ZIP 导入在创建 `AdmZip` 前先检查文件本身大小 | `8707c88` |
| 37 | Markdown 预览截断感知 fenced code block 边界，避免切断代码围栏；展开状态在流式输出中保持 | `83eb2ca` |
| 19 | 补充 Markdown 截断、Hook 正则风险、SHA256、状态持久化 flush 等单元测试 | `aeca7c9` |

### 仍存在的限制

- **问题 1**：哈希校验依赖 ChangeCard 能获取到 diff 事件中的 `newText`。若 `change_summary` 来自无 diff 的源，则仍跳过校验；后续可在生成 `change_summary` 时统一写入 `snapshotHash`。
- **问题 22**：焦点陷阱依赖弹窗组件使用 `useDialogFocus`；未接入该 Hook 的自定义浮层仍需单独处理。
- **问题 35**：当前为静态风险模式检测，未使用 RE2 或 Worker 执行正则；理论上仍可能存在未覆盖的灾难性回溯模式。
- **问题 14（macOS 签名和公证）**：属于 CI/secrets 配置层面的发行风险，未在代码层面改动，仍待运维确认。

### 验证结果

- `pnpm build`：通过
- `pnpm test:run`：通过，57 个测试文件、410 项测试
...
1. 由产品/负责人确认剩余限制的接受度
2. 若需彻底消除问题 1 的盲区，在 `eventMapper` 生成 `change_summary` 时统一写入 `snapshotHash`
3. 若需彻底消除问题 35 的回溯风险，引入 RE2 或子进程/Worker 执行 matcher
4. 由运维确认 macOS 签名、公证与 CI secrets 配置
