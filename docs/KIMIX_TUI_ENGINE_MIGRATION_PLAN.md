...
| 会话恢复 | `kimi -S <id>` / `-C`；侧栏切官方 session 索引 | ⬜ 待接（D-2） |
...
- **D-2 会话恢复 `-S`/`-C` —— ⬜ 待接线**
  - 传输层 `StartTuiSessionRequest.args` → spawn 已就绪，kimi 0.6.0 支持 `-S/-C`，
    但调用层（`Composer.tsx`/`TuiDebugPanel.tsx` 的 `startTuiSession`）均未传 args。
  - 需：Kimix session 持久化官方 `officialSessionId`（`tuiHost.ts` 已能从屏幕抓取），
    侧栏 `selectSession` 恢复时 `startTuiSession({ workDir, args: ["-S", officialSessionId] })`。
