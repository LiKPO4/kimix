...
| 导出 | `/export [path]` 或 `kimi export <sessionId>`（ZIP） | ⬜ 待接 TUI |
| 导入 | `/import <file>` / `/import <sessionId>` | ⬜ 新增 |
| 会话恢复 | `kimi -S <id>` / `-C`；侧栏切官方 session 索引 | ⬜ 待接 |
| 会话分叉 | `/fork` | ⬜ 新增 |
| 撤销重试 | `/undo` | ⬜ 新增 |
| 压缩上下文 | `/compact [指令]` | ⬜ 新增 |
| 侧问题 | `/btw <question>`（隔离上下文、禁用工具） | ⬜ 新增 |
| 标题/重命名 | `/title` `/rename` | ⬜ 新增 |
| 工作区目录 | `/add-dir <path>` | ⬜ 待接 |
| 用量配额 | `/usage`（仅 Kimi Code 平台） | ⬜ 待接 |
| Hooks 页 | 官方 `/hooks` 查看；Kimix prompt-submit hooks 执行需迁移到 TUS 注入前 | ⬜ 待迁移 |
...
### 阶段 D：会话生命周期接入
- `/fork` `/undo` `/compact` `/import` `/title` `/add-dir` 经 `sendTuiInput` 接入对应 UI。
- 侧栏切到官方 `session_index.jsonl` + `kimi -S/-C` 恢复。
- 验收：分叉/撤销/导入各跑通一次，侧栏新会话带官方 sessionId。

### 阶段 E：长程任务迁移到 /task
- LongTasksPanel 改对接官方 `/task` 后台任务，淘汰 prompt-mode longTaskService 编排。
- 验收：后台任务在官方 `/task` 浏览器与 Kimix 页双向一致。

### 阶段 F：Hooks 迁移
- Kimix prompt-submit hooks 在 `sendTuiInput` 注入前执行；查看接 `/hooks`。
- 验收：hook 规则在 TUI 发送链路生效，无 prompt-mode 依赖。
