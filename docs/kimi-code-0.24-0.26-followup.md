...
### 7. 运行中快照对账提前关闭活助手（流式四症状）

- v2 快照会把**进行中轮次**已提交步骤的内容作为 `isComplete: true` 的助手带回；`reconcileRunningKimiSnapshot` 把它 `mergeEvents` 进本地未完成助手并将其关闭——每 4 秒静默期触发一次运行中采样，表现为：每段结束即显示"输出完成"、过程详情周期折叠、旧过程内容"消失"只剩最新、消息头闪动；若关闭后模型进入纯工具静默期，消息头彻底不显示（该轮唯一的助手事件已是完成态，只渲染折叠行）。
- 修复：只要本地还有未完成助手，当前轮次（最后一条用户消息之后）的 canonical 助手一律**按未完成**合并——完成态只能由真实 turn 结束事件授予；内容已被本地覆盖的视为已挂载跳过。回归测试覆盖"同文跳过/更长补齐/旧轮次不动"三种情形。
- 另一实测事实：用户轮次 `turn.ended.durationMs=616s`，而 `prompt()` 对 `prompt.completed` 的硬等待只有 180s，长轮次必报"WebSocket 等待超时"。修复：等待改为活跃度感知——目标会话任何帧重置 180s 空闲计时，流转未死不再误杀。

### 8. 静默判活与错误恢复链

- 180s 空闲超时后不再直接报错卡：先查官方 `/status`，会话仍运行（running/awaiting_*）就继续等；确认不运行才用快照补齐并按完成收口。v2 轮次在超长工具/无增量阶段可能数分钟无帧，"静默≠死亡"。
- 重试上一条成功派发后现在会移除错误卡（此前错误事件永久留在消息链里，只能手点 X）。
- 运维教训（本次排查关键干扰项）：**安装版 `D:\software\kimix\Kimix.exe` 与开发版共用同一 `%APPDATA%\kimix` profile 和同单例 Kimi Server**。并存症状：GPUCache"拒绝访问"、profile 锁死、渲染进程崩溃、官方目录混入对方会话（探针/测试会话污染）、排障证据互相污染。验证改动前必须先关掉安装版（`taskkill /F /IM Kimix.exe /T`），且 `taskkill electron` 后要等进程真正退出再清理缓存。
- 环境事实：本机 profile 曾因反复强杀导致渲染进程启动即崩（exit 143 静默退出，无错误日志）；清 `IndexedDB` 以外全部 Chromium 缓存目录（GPUCache/Cache/Code Cache/Dawn*）后恢复。profile 损坏排查可用 `--user-data-dir=<临时目录>` 对照。
