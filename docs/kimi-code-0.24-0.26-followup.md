...
- v2 快照会把**进行中轮次**已提交步骤的文本作为 `isComplete: true` 的助手带回；`reconcileRunningKimiSnapshot` 的去重判定要求 complete 状态一致，于是 canonical 完整助手被 `mergeEvents` 合并进本地未完成助手并将其关闭——每 4 秒静默期触发一次运行中采样，表现为：每段结束即显示"输出完成"、过程详情周期折叠、旧过程内容"消失"只剩最新、消息头闪动；轮次结束才完整显示。
- 修复：去重判定扩展为——canonical complete 助手的内容若已被本地**未完成**助手覆盖（归一化包含），视为已挂载并跳过合并。回归测试先红后绿。
