...
状态：阶段 0-9 与用户实测已完成；发布前全量审查补齐并发持久化屏障后，进入 v2.16.0 正式发布流程。
...
- 每次房间投递都在同一隐藏包内声明当前接收者身份；房间成员被定义为独立会话协作者，而不是当前 Agent 扮演的多个角色。即使本轮不补充正文也保留这条身份协议。
- 桥接正文逐条标记“用户消息”或“其他独立 Agent 消息”并携带发言者；目标 Agent 引用同伴结论时必须按名称归因，不得把同伴经历描述为自己完成。
...
19. 房间模型 payload 必须声明当前接收者；其他 Agent 的历史正文只能作为署名同伴消息进入，不能暗示为当前 Agent 的历史身份或经历。
...
不提供实施者、审查者等预设行为角色或 persona；Kimix 只注入不可配置的房间身份归属协议，用来区分当前接收者与其他独立 Agent 的署名消息，不规定 Agent 应如何工作。
...
- [x] startup、后台 repair、running snapshot 和 resume 对每个 Agent 独立选择 runtime/official session。
- [x] 一个 Agent 恢复失败时保留其 canonical history 和错误状态，其他 Agent 继续恢复和使用。
- [x] Server -> SDK 迁移更新对应 Agent 绑定和 activity，不改变房间或其他 Agent。
- [x] Provider/model alias 缺失时标记 unavailable，不静默切换模型。
...
- [x] 移出 Agent 不删除历史，默认转换为独立会话并保留原官方 session。
- [x] 归档/恢复逐 Agent `Promise.allSettled`，记录每个 Agent 的成功、失败和可重试状态。
- [x] 部分失败时房间保持可见，不写“已全部归档/恢复”。
- [x] 房间任一 Agent 活跃时禁止添加、移出、归档和会改变身份归属的操作。
...
截至 2026-07-13，阶段 0-4 已完成。阶段 3 已实现并验证：
