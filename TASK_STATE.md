...
- 残留已知项（量级小，不修）：首次进入大会话一次性 parts 合并 ~2.3s（缓存只省重复）；persist 全量 46 会话 100ms/次；小会话 repair 对账 <100ms。
- 方法论沉淀（已入 knowledge log）：perfDiag 子相位 + counts 计数是逐层定位的唯一手段——「看起来该生效的机制」失效时，先给机制的决策点埋点再改。
- 212 发版后本地已累积 213~218 六个 patch 未推送；用户确认流畅后可准备 v2.21.218 发版（release notes 覆盖 212 之后）。
...
- 修复：DialogSystem 两处链接按钮移除 disabled（本体卡、Kimi Code 卡）；两张卡片主操作按钮（升级/更新中）的禁用逻辑保持不变。
- 验证：typecheck 干净；全量 203 文件 2256 用例全过；build 通过。实机待用户确认。
...
- 主进程：新增「下次启动自动录制」一次性标记（userData/diagnostics/auto-record.json，独立小文件、不依赖设置同步链）；app.whenReady 链最前触发 startStartupAutoRecordingIfNeeded()——窗口创建前即开始录制，时长固定 60s；一次性标记录完即清；另支持 --kimix-auto-record 参数或 KIMIX_AUTO_RECORD=1 强制本次录制（界面完全点不动时的兜底路径）。
- 录制完成通知：启动自动录制收尾后弹系统通知（含文件路径，点击 showItemInFolder 直接定位文件）。
- 渲染层：diagRecorder 状态增加 autoArm；setDiagRecordingAutoArm 开关；启动/挂载对账自动接管（帧间隔等采集继续生效）。
- 设置页：日志录制小节新增「下次启动自动录制」开关（一次性，启动录制时由主进程自动关闭）。
- 验证：typecheck 干净；全量 203 文件 2255 用例全过（新增 4 条）；build 通过。实机待用户在公司电脑验证。
...
- notes 落 docs/release-notes/v2.21.202.md 并同步根目录 fallback；推送 master + tag 触发 CI：Release run 34761608963 全绿（win/mac/linux + publish），产物 17 个齐全，body 正确取自版本 notes；Knowledge run 34761605035 全绿。
