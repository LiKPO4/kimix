...
- 已完成：新增短时用户滚动意图判定，内容 shrink/clamp 不再误杀 auto-follow；启动首会话以 `isLoading` 标记官方历史恢复阶段；隐藏及初始 settle 窗口内强制当前 28 个消息块同步渲染真实 Markdown DOM，再基于真实高度置底显示；版本号同步到 v2.12.54。
- 关键文件：`src/components/chat/ChatThread.tsx`、`src/utils/scrollIntent.ts`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；eager Markdown 后的首会话最终高度待用户实机验收。
- 下一步：用户完全退出并启动 v2.12.54，确认首会话正文出现时已位于最终底部，且 3-5 秒内不再二次下移。
