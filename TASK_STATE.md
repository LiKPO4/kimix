...
## 2026-07-02 v2.12.57 首次会话自动置底
- 当前目标：修复首次尾部预览的消息正文恢复增高时，视口先悬在中段再由 JS 重新置底的问题。
- 根因：减少到末尾 4 个消息项并未改变普通滚动容器“内容向下增长”的坐标系；初次置底后，恢复中的正文继续增高会把底部推远，`ResizeObserver` 只能在下一帧追赶，产生可见跳动。
- 已完成：首次尾部预览改用反向滚动坐标，`scrollTop=0` 天然表示底部，正文增高时向上生长；所有底部距离判断兼容反向坐标；用户主动加载较早历史后仍切回普通坐标并按新增高度补偿视口；版本号同步到 v2.12.57。
- 关键文件：`src/components/chat/ChatThread.tsx`、`src/utils/scrollIntent.ts`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；底部原点布局与按需 prepend 的视口保持待用户实机验收。
- 下一步：完成测试、生产构建和提交后，由用户完全退出并启动 v2.12.57，确认尾部正文恢复期间底边不动；向上滚或点击提示后，当前尾部消息位置保持稳定。
