...
- 已完成：新增短时用户滚动意图判定；启动与切换会话采用 bottom-first，先渲染末尾 4 个消息项并贴底；取消官方历史收束后的自动扩展，只有用户向上滚动或点击提示时才 prepend 较早消息，并按新增高度补偿 `scrollTop`；版本号同步到 v2.12.56。
- 关键文件：`src/components/chat/ChatThread.tsx`、`src/utils/scrollIntent.ts`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；按需 prepend 的锚点保持待用户实机验收。
- 下一步：用户完全退出并启动 v2.12.56，确认后台恢复不再触发中段跳转；向上滚或点击提示后，当前尾部消息位置保持稳定。
