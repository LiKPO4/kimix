...
- 已完成：新增会话切模 IPC；已配置别名与 Server 目录归一化并按 Provider 分组；底栏弹窗支持搜索、当前项、加载/空态/失败态和管理模型入口；运行中禁用切换；会话模型优先显示；切模空闲状态不再污染上一轮消息；第三方 OpenAI 模型缺少 `max_output_size` 时经官方配置接口补为 65536；版本号同步到 v2.12.26。
...
- 验收：模型与事件局部测试 30/30、全量测试 276/276、知识库校验、`git diff --check` 与生产构建均通过；用户已完成 v2.12.25 弹窗实机验收并反馈两处链路问题，本轮修复待 v2.12.26 再验。
