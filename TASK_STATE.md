...
- 当前修复：v2.10.10 从复现会话日志确认 SDK 首轮固定 30 秒等待来自官方 `plugin-kimi-datasource:data` MCP 启动超时，模型请求在超时后才开始。Kimix vendored SDK 将未显式配置的 MCP 启动上限由 30 秒收紧为 4 秒；声明了 `startupTimeoutMs` 的服务不受影响，并通过 vendor 脚本持久保留该策略。v2.10.9 的会话配置同值短路继续保留，但已确认不是本次固定 30 秒的主因。
