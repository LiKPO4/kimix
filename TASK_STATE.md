...
- 下一步：进入阶段 5：在 Composer `+` 菜单加入 gated“添加 Agent”，复用现有模型目录和权限，完成名称/mention 校验、普通会话原地升级、单接收者选择与移出入口；保持普通单 Agent UI 不变。
