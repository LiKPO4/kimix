...
- 修复：保留 12px 拖拽热区，用对称 `margin-inline: -6px` 让其覆盖在真实分界线两侧，flex 净占位归零；增加 z-index 保证热区与可见线位于相邻面板之上。共用样式同时修正左侧栏和右侧检查器拖拽柄；侧栏收起时的 12px spacer 是非拖拽布局，本轮不改。
- 回归：新增 CSS 契约测试，锁定“12px 热区 + 负外边距抵消占位 + 层级”；定向 1/1 与 Node/Renderer TypeScript 检查通过。
