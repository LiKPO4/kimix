...
- 已完成：新增短时用户滚动意图判定，内容 shrink/clamp 不再误杀 auto-follow；启动首会话以 `isLoading` 标记官方历史恢复阶段，聊天正文保持隐藏，恢复成功或失败收束后在 layout 阶段先置底再显示；版本号同步到 v2.12.53。
- 关键文件：`src/components/chat/ChatThread.tsx`、`src/utils/scrollIntent.ts`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；首会话恢复期间的 reveal gate 待用户实机验收。
- 下一步：用户完全退出并启动 v2.12.53，确认首会话不会先显示缓存中的中间高度，而是正文出现时已位于最终底部。
