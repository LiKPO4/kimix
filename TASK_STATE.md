...
- 下一步：实现房间用户消息和 Agent turn 的稳定时间线投影，并把 canonical snapshot/undo 收口为 Agent scoped reconcile 入口。
