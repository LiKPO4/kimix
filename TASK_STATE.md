...
- 未完成：阶段 5–6 待实施；每阶段单独验证、检查 diff 并提交。阶段 1–4 尚待用户用真实单轮长回复观察流畅度和完成瞬间样式切换。
...
- 下一步：提交阶段 4；随后执行阶段 5，收敛流式更新时重复的滚动跟随与尺寸测量。
