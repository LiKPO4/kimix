...
## 2026-07-02 v2.12.58 首次会话真实尾部恢复
- 当前目标：避免启动时先展示本地旧缓存的“伪底部”，几秒后再切换到官方历史的真实底部。
...
- 补充根因：Server snapshot 消息本身包含官方 `created_at`，但 Kimix 转换历史 frame 时丢弃了该字段，事件映射最终回退到 `Date.now()`，导致昨晚消息在今早加载后显示成今早时间；`seq` 也曾被错误当作时间兜底。
- 已完成：有缓存的官方会话处于首轮恢复时只显示贴近输入区的“正在同步最新会话”状态，不暴露旧缓存尾部；官方恢复任务取消 1.2 秒延迟并立即启动；snapshot 拆分出的用户、思考、正文、工具及结束 frame 全部透传消息 `created_at`，并移除 `seq` 时间兜底；真实历史到达后沿用底部原点尾部布局一次性显示；版本号同步到 v2.12.58。
- 关键文件：`src/App.tsx`、`electron/kimiCodeServerClient.ts`、`electron/kimiCodeHost.ts`、`src/components/chat/ChatThread.tsx`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；真实尾部切换待用户实机验收。
- 下一步：验证并提交后，由用户完全退出并启动 v2.12.58，确认启动时只出现短暂同步状态，随后直接进入 2026-07-02 09:06 的真实尾部。
