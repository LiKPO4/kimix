...
- 下一步：只实施阶段 4F：升级备份 schema 2，完整校验和保存 collaboration；导入副本时重映射房间/Agent/message/turn/delivery/队列引用并清空全部官方 runtime/catalog 绑定；不同时开放 UI。
