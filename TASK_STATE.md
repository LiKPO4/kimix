...
- 已完成：新增短时用户滚动意图判定；启动与切换会话采用 bottom-first，先渲染末尾 4 个消息项并贴底，官方历史收束后在 layout 阶段向上补齐原有最多 28 项；尾部 Markdown eager 渲染真实 DOM；版本号同步到 v2.12.55。
- 关键文件：`src/components/chat/ChatThread.tsx`、`src/utils/scrollIntent.ts`。
- 验收：全量测试 40 文件 288/288、知识库校验、`git diff --check` 与生产构建通过；bottom-first 的首会话视觉过程待用户实机验收。
- 下一步：用户完全退出并启动 v2.12.55，确认立即看到末尾消息，较早历史补齐时底部保持不动。
