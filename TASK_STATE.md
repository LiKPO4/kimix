...
- 下一步：只实施阶段 4E：在网络调用前持久化 room message、接收者顺序、agentTurnId、dispatchAttemptId 和 queued 状态；官方接受后记录稳定身份，发送后确认前崩溃进入 indeterminate，禁止自动重发；不同时开放 UI。
