...
- 已完成：完成 16 个提交的静态审查。第 1 项已完成：runtime token 改为放在 electron-vite 的 `--` 参数边界之后。第 2 项已完成：子代理正文提升日志按事件去重，主进程诊断文件改为异步串行队列。第 3 项已完成：普通诊断统一脱敏正文、工具载荷、路径、stack、快照和 base64，只保留结构元数据；完整 200-event 快照仅在启动环境显式设置 `KIMIX_DETAILED_DIAGNOSTICS=1` 时由 renderer 序列化。第 3 项定向 38 项测试、typecheck、生产构建通过。
- 待办与执行顺序：
...
- 下一步：执行第 4 项，修复通知跨会话切换时丢失 timeline focus 请求。
