...
## 2026-07-02 v2.12.59 首次会话真实尾部恢复
...
- 已完成：本地缓存恢复进入 store 时立即将活动会话标记为同步中；后台 repair 排除活动会话，启动主恢复成为唯一历史写入者；SDK wire message 包装记录透传消息自身或外层原始时间；保留 v2.12.58 的即时官方恢复、Server snapshot `created_at` 透传和底部原点布局；版本号同步到 v2.12.59。
- 关键文件：`src/App.tsx`、`electron/kimiCodeServerClient.ts`、`electron/kimiCodeHost.ts`、`src/components/chat/ChatThread.tsx`。
- 验收：时间与回放链路局部测试 3 文件 95/95、全量测试 40 文件 289/289、知识库校验、`git diff --check` 与生产构建通过；启动单次切换待用户实机验收。
- 下一步：全量验证并提交后，由用户完全退出并启动 v2.12.59，确认旧缓存不再露出、真实尾部只切换一次、消息元信息稳定保留 20:03:27。
