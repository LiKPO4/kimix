...
- 已完成：完成运行态、持久化历史和 UI 路由审计；确定不新增外部 Runtime/Provider 体系、不预设身份、不增加独立模式；持久化完整实施计划与 OKF 架构决策；建立 `codex/multi-agent-room` 功能分支；新增 CollaborationState、RoomAgent、房间消息/delivery 和事件归属字段；旧 Session 可无损映射为 synthetic primary；新增 `roomId + roomAgentId` 活动表，流事件按 Agent 分批，事件、状态、终态、quiet snapshot、轮询和 Server -> SDK 迁移均先解析 runtime owner；完成并提交阶段 3；完成阶段 4A 的防御性 collaboration 持久化、旧版本 primary 回写恢复、逐 Agent settle 和递归图片保存；完成阶段 4B，renderer 只能提交严格 roomMetadata，主进程生成官方字段并用稳定 roomAgentId 请求 session，创建前查询包含空 session 的官方目录，唯一匹配直接恢复，重复或已归档停止自动创建，metadata 贯穿 Server、SDK 和 fallback；完成阶段 4C，受控 metadata 只在 schema、路径、房间、primary 和 Agent 身份唯一匹配时折叠并重绑次要 session，同批主、次目录仍只保留一个房间镜像，歧义与孤儿保持独立可见，Server 只标记缺失 Agent、SDK 非权威目录不下删除结论；完成阶段 4D，startup 并行恢复全部房间 Agent，后台 repair、running snapshot 和 resume 复用逐 Agent runtime/official 候选，单 Agent 失败保留 canonical history 和恢复错误，Server -> SDK 迁移同步目标 Agent activity，Provider/model alias 缺失标记 unavailable 并阻止静默切换；完成阶段 4E，逐 delivery 在网络前依次持久化 queued/sending 和稳定 attempt/turn 身份，网络结果不确定进入 indeterminate 且不自动重发，startup/repair 只凭稳定官方证据恢复，显式重试保留旧尝试审计；完成阶段 4F，会话备份升级 schema 2 并完整保存 collaboration/Agent 活动引用，schema 1 维持单 Agent 导入，损坏引用与未来 schema 拒绝写回，分叉副本全量重映射房间/Agent/message/turn/attempt/事件/活动/队列和 active context 并清空全部官方绑定，tombstone 覆盖所有 Agent runtime/official ID；完成阶段 4G，逐 Agent 归档/恢复记录部分失败并支持精确重试，成功/失败状态不再冒充整房间结果，活跃房间禁止归档和成员变更，移出的次要 Agent 保留历史并转为独立官方会话；完成阶段 5A 创建/投递底座，新增 gated UI 开关、Agent 草稿校验和 provisioningError，runtime 状态会把 accepted delivery 结算为 running/等待/终态，accepted 也纳入成员生命周期门禁；阶段 5A 局部 14 项测试与生产构建通过。
- 未完成：阶段 5 Composer 添加 Agent、单接收者选择/移出和真实发送接线；阶段 6 精确 mention、多目标和独立队列；Agent scoped 操作；搜索/导出以及视觉和真实流程验收。
...
- 下一步：继续阶段 5：把 `AddRoomAgentDialog` 接入 Composer，严格执行本地意图持久化后再创建官方 session；随后接单接收者选择、移出和 `dispatchQueuedRoomDelivery` 发送路径，保持 gate 关闭时普通单 Agent UI 不变。
