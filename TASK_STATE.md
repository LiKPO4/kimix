...
- 当前修复：v2.10.9 根据官方 wire 时间线确认老 SDK 会话的慢首响分为“模型步骤前准备约 29.7 秒 + 模型首 Token 约 17.5 秒”；恢复链路会重复写入未变化的模型、权限、思考和 Plan 配置。主进程现缓存已生效配置并跳过同值写入，避免恢复/预热/发送连续改写 SDK 会话配置、破坏大上下文缓存。
