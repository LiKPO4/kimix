...
import { buildChatCompletionsUrls, probeThinkingEfforts, resolveCatalogModelMetadata } from "../providerEffortProbe";
...
describe("resolveCatalogModelMetadata", () => {
  const providers = [
    {
      providerId: "opencode-go",
      baseUrl: "https://opencode.ai/zen/go/v1",
      models: [{ id: "hy3", maxContextSize: 256000, supportEfforts: ["off", "low", "high"] }],
    },
    {
  ...
  it("按 provider id 返回 opencode-go/hy3 的模型级声明", () => {
    expect(resolveCatalogModelMetadata({
      providerName: "opencode-go",
      baseUrl: "https://proxy.example/v1",
      modelId: "hy3",
      providers,
    })).toEqual({ status: "resolved", providerId: "opencode-go", maxContextSize: 256000, supportEfforts: ["off", "low", "high"] });
  });
  ...
  it("模型未声明思考档位时仍返回 Context", () => {
    expect(resolveCatalogModelMetadata({
      providerName: "opencode-go",
      modelId: "mimo-v2.5",
      providers: [{ ...providers[0], models: [{ id: "mimo-v2.5", maxContextSize: 1_000_000 }] }],
    })).toEqual({ status: "resolved", providerId: "opencode-go", maxContextSize: 1_000_000 });
  });
});
