...
## 7. Codex 参考图 UI 修复执行计划（2026-05-08）

### 背景

当前 Kimix UI 与 `codex参考图1.png`、`codex参考图2.png`、`codex参考图3.png`、`codex参考图4.png` 对比后，主要问题是：整体布局偏拥挤、字体偏小、输入框 focus 样式和内部布局异常、消息操作重复且包含不需要的点赞、排队区域不是真实队列、顶部菜单只有提示没有下拉内容。

### 参考图设计要点

1. 主内容区采用居中的白色大圆角容器，外层保留暖色背景和舒适边距。
2. 聊天内容与输入框使用统一最大宽度，不贴边，不铺满到窗口右侧。
3. 输入框是底部居中的大圆角 composer，上层输入区、下层工具栏，focus 后不出现浏览器默认蓝色边框。
4. 排队消息显示在输入框上方，形成真实列表，每条队列项有操作入口。
5. Agent 思考内容应折叠在同一条回复内，不应和正式回复拆成两条都可点赞/复制的消息。
6. 顶部 `文件 / 编辑 / 查看 / 窗口 / 帮助` 应有下拉菜单内容，而不是只显示 toast。

### 执行阶段

#### 阶段 1：重做输入框布局并去除 focus 蓝框
...
- 将 composer 改成参考图中的两层结构：上层 textarea，下层工具栏。
- 容器居中并设置统一最大宽度。
- 移除 textarea 默认 focus 蓝色边框，改用弱边框或轻阴影表示激活状态。
- 修复权限、模型、语音、发送/停止等按钮溢出问题。
- 发送按钮保持右下圆形视觉，运行中切换为停止按钮。
...
- 将 `pendingMessages: string[]` 升级为结构化队列。
- 队列显示在输入框上方，而不是只显示计数。
- 每条队列项支持：立刻发送、修改、删除、调整顺序。
- 正在运行时发送的新消息进入队列，不丢失。

验收：

- 排队消息真实显示在 composer 上方。
- 每条排队消息有操作按钮。
- 删除、修改、调整顺序可用。

#### 阶段 4：合并思考/回复展示，移除点赞点踩
...
- 思考内容作为同一条 assistant 消息中的可折叠区域。
- 正式回复和思考不拆成两条独立可操作消息。
- 删除点赞和点踩按钮，只保留复制。
- 复制按钮改为弱显示，hover 时更明显。

验收：

- 同一轮 Agent 的思考与正文在同一条消息中展示。
- 不再出现点赞/点踩入口。
- 每条消息只保留复制相关操作。
...
- 将当前 toast-only 顶部按钮改成真实下拉菜单。
- 菜单包括：文件、编辑、查看、窗口、帮助。
- 未实现的功能显示 disabled 或“即将上线”，不再只有 toast。
- 支持点击空白处关闭、切换菜单。
...
1. 阶段 1：重做输入框布局并去除 focus 蓝框。
2. 阶段 2：调整整体布局密度、字体和间距。
3. 阶段 3：实现真实排队列表。
4. 阶段 4：合并思考/回复展示，移除点赞点踩。
5. 阶段 5：恢复顶部菜单下拉内容。
6. 最后运行 `pnpm build` 做 TypeScript/Vite 构建验证；如需界面验收，再运行 `pnpm dev` 人工比对参考图。
