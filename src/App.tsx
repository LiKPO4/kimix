...
async function repairKimiCodeHistoryBodies(sessions: Session[], options?: { includeActive?: boolean }) {
      ...
      for (const sessionId of target.sessionIds) {
        ...
        useSessionStore.setState((state) => ({
          sessions: state.sessions.map((item) => {
            ...
            if (!shouldReplaceWithCanonicalKimiHistory(localEvents, reconciliation.events, { sessionId: item.id, roomAgentId: target.roomAgentId, reason: "repair", rawCanonicalEvents: canonicalEvents, forceCanonical })) {
  ...
  reconciliation.events,
      ...
      if (historyReachable && !recoveryAccepted && !circuitSkipped && session.collaboration) {
        useSessionStore.setState((state) => ({
          sessions: state.sessions.map((item) => (
            item.id === session.id
    ...
    sessions: state.sessions.map((session) => {
      ...
      for (const result of results) {
        ...
        const rejectedBaseEvents = canonicalAdopted
          ? localEvents
          : mergeCanonicalFragmentTurnBodies(collapseDuplicateMaterializations(localEvents), reconciliation.events, { sessionId: reconciliation.session.id, roomAgentId: result.target.roomAgentId, reason: "runtime-recovery" });
  ...
  const dispatchNextPendingKimiMessage = async (uiSessionId: string, runtimeSessionId: string) => {
    ...
    const timer = setTimeout(() => {
      sendKimiCodePromptWithRetry({
        ...
        // 渲染层无法区分消息"未发出"与"已发出但确认丢失"，失败时保守重排到队首，保持其余消息原有顺序。
        logEvent("app.pendingDispatchRequeue", { uiSessionId, error: message.slice(0, 200) });
        useSessionStore.getState().requeuePendingMessageFront(next);
            ...
            try {
              ...
              if (runtimeOwner) {
                ...
                const rejectedBaseEvents = !reconciliation.applied || canonicalAdopted
                  ? localAgentEvents
                  : mergeCanonicalFragmentTurnBodies(collapseDuplicateMaterializations(localAgentEvents), reconciliation.events, { sessionId: latestOwner.id, roomAgentId: ownerAgentId, reason: "startup" });
    ...
    const unsubscribeKimiCodeStatus = window.api.onKimiCodeStatus((payload) => {
      ...
      if (roomAgentId) {
        setRoomAgentActivity({
          ...
          // terminal 状态显式清掉回合身份（浅合并下显式写 undefined 才会覆盖），避免迟到事件继承到错误回合。
          roomMessageId: undefined,
          activeTurnId: undefined,
          updatedAt: Date.now(),
        });
        ...
        void shouldWaitForOfficialPromptQueue(payload.sessionId).then((shouldWait) => {
          ...
          const timer = setTimeout(() => {
            ...
            sendPromise.then((res) => {
              ...
              // 渲染层无法区分消息"未发出"与"已发出但确认丢失"，失败时保守重排到队首，保持其余消息原有顺序。
              logEvent("app.queueDispatchRequeue", { uiSessionId, error: message.slice(0, 200) });
              useSessionStore.getState().requeuePendingMessageFront(next);
              if (isKimiActiveTurnError(message)) {
                updateSession(uiSessionId, (session) => {
                  const filteredEvents = session.events.filter((event) => event.id !== placeholderId && event.id !== userEventId);
