...
describe("自定义风格 CSS 角色消费契约", () => {
  it("公共弹层外壳的强制基础声明直接消费自定义角色并保留默认回退", () => {
    ...
    expect(css).toMatch(/\.kimix-diff-panel\s*\{[^}]*border:\s*var\(--ui-role-inspector-resting-border,\s*1px solid var\(--border-subtle\)\)\s*!important;[^}]*background:\s*var\(--ui-role-inspector-resting-background,\s*var\(--surface-base\)\)\s*!important;[^}]*border-radius:\s*var\(--ui-role-shell-radius,\s*var\(--kimix-window-corner-radius\)\)\s*!important;/s);
    expect(css).toMatch(/\.kimix-longtask-inspector\s*\{[^}]*border:\s*var\(--ui-role-inspector-resting-border,\s*1px solid var\(--border-subtle\)\)\s*!important;[^}]*background:\s*var\(--ui-role-inspector-resting-background,\s*var\(--surface-base\)\)\s*!important;[^}]*border-radius:\s*var\(--ui-role-shell-radius,\s*var\(--kimix-window-corner-radius\)\)\s*!important;/s);
