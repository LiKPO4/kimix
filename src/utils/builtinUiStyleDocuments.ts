...
export function buildUiStyleAiPrompt() {
  ...
  return [
    ...
    "4. basedOn 只能是 default、modern、retro、nostalgia 之一。",
    "5. roles 必须完整保留下列全部角色，不能删除或新增角色：",
    UI_STYLE_ROLE_IDS.join(", "),
    "6. radius 引用只能是 small、medium、large、card、panel、shell、pill。",
    "7. surface 只能是 transparent、ground、base、elevated、hover、active。",
    "8. border 只能是 none、subtle、default、strong；elevation 只能是 none、control、card、popup、field。",
    "9. elevation.kind 只能是 flat、raised、floating、inset；所有数值必须保持在模板展示的合理量级内。",
    "10. 完成 JSON 后，必须优先使用你可用的文件工具在当前工作目录创建 `kimix-ui-style-<id>.json`；`<id>` 使用最终 JSON 中的 id。文件必须是 UTF-8 编码的纯 JSON，不得包含 Markdown 代码围栏或 JSON 注释。",
    "11. 文件创建成功后，不要在对话中重复整份 JSON，只需简洁说明已创建并给出可点击或可复制的文件路径。",
    "12. 只有当当前环境确实没有文件写入能力时，才允许降级为在对话中输出一个 JSON 代码块，并明确说明未能创建文件。",
    "",
    "请以这份完整模板为结构生成结果：",
    "```json",
