...
function installBrowserPreviewApi() {
  ...
  const previewApi: WindowAPI = {
    ...
    getKimiCodeServerStatus: (): Promise<KimiCodeServerStatusResponse> => Promise.resolve({
      success: true,
    ...
    getThinkingTranslationCredentialStatus: (): Promise<ThinkingTranslationCredentialStatusResponse> => Promise.resolve({
      success: true,
      data: { configured: false, storageAvailable: false },
    }),
    saveThinkingTranslationCredential: (): Promise<VoidResponse> => fail("保存 Microsoft Translator 凭据"),
    clearThinkingTranslationCredential: (): Promise<VoidResponse> => fail("清除 Microsoft Translator 凭据"),
    testThinkingTranslation: (): Promise<ThinkingTranslationResponse> => Promise.resolve({
      success: false,
      error: { code: "credential_missing", message: unsupported("测试 Microsoft Translator 连接") },
    }),
    translateThinking: (): Promise<ThinkingTranslationResponse> => Promise.resolve({
      success: false,
    ...
    preflightKimiCodeTower: () => fail("检查 Kimi Code Tower"),
    getKimiCodeTowerSnapshot: () => fail("读取 Kimi Code Tower 状态"),
    setKimiCodeTowerMode: () => fail("切换 Kimi Code Tower 模式"),
    teardownKimiCodeTower: () => fail("清理 Kimi Code Tower 工作树"),
    ...
    getKimiCodeExtraSkillDirs: () => Promise.resolve({ success: true, data: [] }),
    setKimiCodeExtraSkillDirs: (req) => Promise.resolve({ success: true, data: req.dirs }),
    chooseKimiCodeSkillDirectory: () => Promise.resolve({ success: true, data: { canceled: true } }),
