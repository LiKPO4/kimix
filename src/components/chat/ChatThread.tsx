...
export function shouldRetryAsRoomDelivery(
  isCollaborationRoom: boolean,
  ...
  switch (event.type) {
    ...
    case "error":
      return (
        <ErrorCard
          event={event}
          onRetry={shouldRetryAsRoomDelivery(Boolean(isCollaborationRoom), event)
            ? async () => {
                await requestRoomDeliveryAction({
                  action: "retry",
                  sessionId,
