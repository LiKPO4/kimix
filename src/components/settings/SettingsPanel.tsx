...
export function SettingsPanel({ variant = "modal", onBackToChat }: { variant?: "modal" | "workspace"; onBackToChat?: () => void }) {
  ...
  const [officialArchivedSortMode, setOfficialArchivedSortMode] = useState<ArchivedSortMode>("archived");
  const [officialArchivedWorkspace, setOfficialArchivedWorkspace] = useState("all");
  const [lastRestoredOfficialId, setLastRestoredOfficialId] = useState<string | null>(null);
  ...
  // 子 Agent 模型池（官方 0.36 [secondary_model] 池形态）
  ...
  const [secondaryPoolAddAlias, setSecondaryPoolAddAlias] = useState("");
  const [secondaryPoolAddHint, setSecondaryPoolAddHint] = useState("");
  ...
  const saveSecondaryModelPool = async () => {
    ...
    const res = await window.api.saveKimiCodeSecondaryModelPool({
      ...
      force: secondaryPoolForce,
      // 锁定模式与池条目互斥（官方校验同样拒绝共存），锁定时提交空池。
      entries: secondaryPoolForce ? [] : secondaryPoolEntries,
    });
  ...
  const scanOfficialKimiThemes = async () => {
    ...
    if (themePalette.startsWith("kimi:") && !activeThemeStillExists) setThemePalette(DEFAULT_THEME_PALETTE_ID);
    if (presets.length === 0) {
  ...
  const handleRestoreOfficialArchivedSession = async (session: KimiCodeArchivedSessionSummary) => {
    ...
    if (localRoom?.collaboration) {
      ...
      const lifecycleReport = formatRoomLifecycleOutcomes("restore", result.outcomes);
      if (!result.success) {
        setOfficialArchivedMessage(`房间「${localRoom.title}」${lifecycleReport}`);
        return;
      }
      forgetArchivedSessionTombstonesByIds([localRoom.id, ...restoredIds]);
      setOfficialArchivedMessage(`房间「${localRoom.title}」${lifecycleReport}。切换到对应项目后会重新同步到侧栏。`);
      return;
    }
    ...
    setLastRestoredOfficialId(session.id);
    setOfficialArchivedMessage(`已恢复「${res.data.title || session.title}」。`);
  };

  const handleOpenRestoredSession = () => {
    if (!lastRestoredOfficialId) return;
    const restored = useSessionStore.getState().sessions.find((item) => (
      item.id === lastRestoredOfficialId ||
      item.runtimeSessionId === lastRestoredOfficialId ||
      item.officialSessionId === lastRestoredOfficialId
    ));
    if (!restored) {
      setOfficialArchivedMessage("该对话尚未同步到本地列表，切换到对应项目后会出现。");
      return;
    }
    const project = useSessionStore.getState().recentProjects.find((candidate) => candidate.path === restored.projectPath);
    if (project) useAppStore.getState().setCurrentProject(project);
    useAppStore.getState().setCurrentSession(restored);
    setLastRestoredOfficialId(null);
    if (variant === "modal") useAppStore.getState().setSettingsOpen(false);
    onBackToChat?.();
  };
  ...
  useEffect(() => {
    const handleFocusSettingsSection = (event: Event) => {
      const sectionId = (event as CustomEvent<{ sectionId?: SettingsSectionId }>).detail?.sectionId;
      if (!sectionId) return;
      focusSettingsSection(sectionId, "center");
    };
    window.addEventListener(SETTINGS_FOCUS_SECTION_EVENT, handleFocusSettingsSection);
    return () => window.removeEventListener(SETTINGS_FOCUS_SECTION_EVENT, handleFocusSettingsSection);
  }, [activeSettingsPageId]);
  ...
  // 本机归档过的会话才有真实归档时刻；其余官方条目的 archivedAt 实为 updated_at。
  const localArchivedAtById = new Map(archivedSessionSummaries.map((item) => [item.id, item.archivedAt] as const));
  const officialArchivedVisible = sortArchivedSessions(
    filterArchivedSessions(officialArchivedSessions, officialArchivedQuery, officialArchivedWorkspace),
    officialArchivedSortMode,
  );
  const officialArchivedGroups = groupArchivedSessionsByWorkspace(officialArchivedVisible);
  const officialArchivedWorkspaceChoices = archivedWorkspaceOptions(officialArchivedSessions);
  const archivedBadgeCount = dedupeArchivedCount(
    officialArchivedSessions.map((item) => item.id),
    archivedSessionSummaries.map((item) => item.id),
  );
        ...
        <div className={`kimix-settings-body ${variant === "workspace" ? "kimix-stable-scrollbar" : ""}`}>
          <div className={`kimix-settings-layout ${variant === "workspace" ? "is-workspace" : ""}`}>
              ...
              <div className="kimix-settings-section" {...settingsSectionProps("archived", 11)}>
                ...
                <div className="kimix-settings-card" style={{ padding: "18px 16px" }}>
                  ...
                  <div className="flex flex-col" style={{ gap: 10, marginTop: 14 }}>
                    <div className="kimix-settings-archive-filters">
                      <div className="kimix-settings-input flex h-9 min-w-0 items-center rounded-lg" style={{ gap: 8, paddingLeft: 12, paddingRight: 12 }}>
                        ...
                        <input
                          value={officialArchivedQuery}
                          onChange={(event) => setOfficialArchivedQuery(event.target.value)}
                          placeholder="搜索已归档会话（标题或项目路径）"
                          className="min-w-0 flex-1 bg-transparent text-[13.5px] outline-none placeholder:text-[var(--kimix-panel-text-muted)]"
                        />
                      </div>
                      <select
                        value={officialArchivedWorkspace}
                        onChange={(event) => setOfficialArchivedWorkspace(event.target.value)}
                        className="kimix-settings-input h-9 rounded-lg text-[13px] outline-none"
                        style={{ paddingLeft: 10, paddingRight: 8, maxWidth: 220 }}
                      >
                        <option value="all">所有工作区</option>
                        {officialArchivedWorkspaceChoices.map((choice) => (
                          <option key={choice.key} value={choice.key}>{choice.label}</option>
                        ))}
                      </select>
                      <select
                        value={officialArchivedSortMode}
                        onChange={(event) => setOfficialArchivedSortMode(event.target.value as ArchivedSortMode)}
                        className="kimix-settings-input h-9 rounded-lg text-[13px] outline-none"
                  ...
                  <div className="flex flex-col" style={{ gap: 14, marginTop: 4 }}>
                    {officialArchivedGroups.length > 0 ? officialArchivedGroups.map((group) => (
                      <div key={group.workspaceKey} className="flex flex-col" style={{ gap: 8 }}>
                        <div className="flex items-center text-[12.5px] leading-5 text-[var(--kimix-panel-text-muted)]" style={{ gap: 8, paddingLeft: 2 }}>
                          <FolderOpen size={13} className="shrink-0" />
                          <span className="truncate">{group.workspaceLabel}</span>
                          <span className="shrink-0">{group.items.length} 个会话</span>
                        </div>
                        {group.items.map((session) => {
                          const trueArchivedAt = localArchivedAtById.get(session.id);
                          const timeLabel = trueArchivedAt
                            ? `归档于 ${formatArchivedTime(trueArchivedAt)}`
                            : `最后活动 ${formatArchivedTime(archivedTimeMs(session))}`;
                          return (
                            <div
                              key={session.id}
                            ...
                            >
                              <div className="flex min-w-0 items-start" style={{ gap: 10 }}>
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-0.5 text-[12px] leading-5 text-[var(--kimix-panel-text-muted)]">{timeLabel}</div>
                                </div>
                              </div>
                              <button
                                type="button"
                      ...
                      <div className="text-[13.5px] leading-6 text-[var(--kimix-panel-text-secondary)]">
                        {officialArchivedLoading
                          ? "正在读取官方归档..."
                          : officialArchivedSessions.length > 0
                            ? "没有匹配的归档对话。"
                            : "官方暂无归档对话。"}
                      </div>
                    )}
                  ...
                  <div className="rounded-xl border border-[var(--kimix-panel-border-soft)] bg-surface-base text-[12.5px] leading-5 text-[var(--kimix-panel-text-secondary)]" style={{ padding: "10px 12px", marginTop: 14 }}>
                    <div className="flex items-center justify-between" style={{ gap: 10 }}>
                      <span className="min-w-0">{officialArchivedMessage || "官方归档状态会在打开设置时自动刷新。"}</span>
                      {lastRestoredOfficialId && (
                        <button
                          type="button"
                          onClick={handleOpenRestoredSession}
                          className="kimix-icon-text-button is-compact shrink-0 text-accent-primary hover:bg-accent-primary-light"
                        >
                          打开该对话
                        </button>
            ...
            <div className="kimix-settings-col">
              ...
              <div className="kimix-settings-section" {...settingsSectionProps("secondaryModel", 4)}>
                <div className="kimix-settings-row-title">
                  <div className="kimix-settings-section-title">
                ...
                <div className="kimix-settings-card" style={{ padding: "16px 18px" }}>
                  <div className="text-[13px] leading-5 text-[var(--kimix-panel-text-secondary)]">
                    主 Agent 派生子 Agent / Swarm 时会参考池内提示挑选模型；提示文案会展示给主 Agent 辅助选择。实验功能，需要 Kimi Code 0.36.0+，由 Kimix 自动开启实验开关。
                  </div>
                  <div className="grid items-center" style={{ gridTemplateColumns: "120px minmax(0, 1fr)", gap: 10, marginTop: 14 }}>
                    <div className="text-[12.5px] leading-5 text-[var(--kimix-panel-text-secondary)]">默认模型</div>
                    <select
                      value={secondaryPoolDefault}
                      onChange={(event) => setSecondaryPoolDefault(event.target.value)}
                  ...
                  <label className="flex items-center text-[13px] leading-5 text-[var(--kimix-panel-text-secondary)]" style={{ gap: 8, marginTop: 12 }}>
                    <input
                      type="checkbox"
                      checked={secondaryPoolForce}
                      disabled={!secondaryPoolDefault}
                      onChange={(event) => setSecondaryPoolForce(event.target.checked)}
                      className="h-4 w-4 accent-accent-primary"
                    />
                    <span>锁定模式：所有子 Agent 强制使用默认模型（不能与池条目共存）</span>
                  </label>
                  {!secondaryPoolForce && (
                    <div style={{ marginTop: 14 }}>
                      <div className="text-[12.5px] leading-5 text-[var(--kimix-panel-text-secondary)]">池条目{secondaryPoolEntries.length > 0 ? `（${secondaryPoolEntries.length}）` : ""}</div>
                      {secondaryPoolEntries.length > 0 && (
                        <div className="flex flex-col" style={{ gap: 8, marginTop: 8 }}>
                          {secondaryPoolEntries.map((entry, index) => (
                            <div key={entry.alias} className="grid items-center" style={{ gridTemplateColumns: "minmax(0, 170px) minmax(0, 1fr) auto", gap: 8 }}>
                              <div className="truncate text-[13px] leading-5 text-[var(--kimix-panel-text)]" title={entry.alias}>{entry.alias}</div>
                              <input
                                value={entry.hint}
                                onChange={(event) => setSecondaryPoolEntries((current) => current.map((item, itemIndex) => (itemIndex === index ? { ...item, hint: event.target.value } : item)))}
                                placeholder="选择提示，如：快速探索、粗活"
                                className="kimix-settings-input h-9 min-w-0 rounded-lg text-[13px] outline-none"
                                style={{ paddingLeft: 10, paddingRight: 10 }}
                              />
                              <button
                                type="button"
                                onClick={() => setSecondaryPoolEntries((current) => current.filter((_, itemIndex) => itemIndex !== index))}
                                className="kimix-icon-text-button is-compact text-text-muted hover:bg-surface-hover"
                                style={{ minHeight: 32 }}
                                aria-label={`移除 ${entry.alias}`}
                              >
                                <Trash2 size={14} />
                              </button>
                            </div>
                      ...
                      <div className="grid items-center" style={{ gridTemplateColumns: "minmax(0, 170px) minmax(0, 1fr) auto", gap: 8, marginTop: 10 }}>
                        <select
                          value={secondaryPoolAddAlias}
                          onChange={(event) => setSecondaryPoolAddAlias(event.target.value)}
                          className="kimix-settings-input h-9 min-w-0 rounded-lg text-[13px] outline-none"
                          style={{ paddingLeft: 10, paddingRight: 10 }}
                        >
                          <option value="">选择模型别名</option>
                          {secondaryPoolModelOptions.filter((alias) => !secondaryPoolEntries.some((entry) => entry.alias === alias)).map((alias) => (
                            <option key={alias} value={alias}>{alias}</option>
                          ))}
                        </select>
                        <input
                          value={secondaryPoolAddHint}
                          onChange={(event) => setSecondaryPoolAddHint(event.target.value)}
                          placeholder="选择提示（展示给主 Agent）"
                          className="kimix-settings-input h-9 min-w-0 rounded-lg text-[13px] outline-none"
                          style={{ paddingLeft: 10, paddingRight: 10 }}
                        />
                        <button
                          type="button"
                          onClick={() => {
                            if (!secondaryPoolAddAlias) return;
                            setSecondaryPoolEntries((current) => [...current, { alias: secondaryPoolAddAlias, hint: secondaryPoolAddHint.trim() }]);
                            setSecondaryPoolAddAlias("");
                            setSecondaryPoolAddHint("");
                          }}
                          disabled={!secondaryPoolAddAlias}
                          className="kimix-icon-text-button is-compact text-accent-primary hover:bg-accent-primary-light disabled:opacity-45"
                          style={{ minHeight: 32 }}
                        >
                          <Check size={14} />
                          <span>添加</span>
                  ...
                  {secondaryPoolMessage && (
                    <div className="text-[12.5px] leading-5 text-text-muted" style={{ marginTop: 12 }}>{secondaryPoolMessage}</div>
                  )}
                </div>
              </div>
