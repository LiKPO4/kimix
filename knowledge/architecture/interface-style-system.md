...
* Retro button hover is one shared interaction language, not a component-by-component exception. Control, navigation-action, navigation-list, menu-trigger, and context-bar roles consume the same Retro hover border/background/shadow tokens; a zero-specificity native-button fallback covers unregistered buttons without overriding semantic danger, primary, selected, or expanded states.
