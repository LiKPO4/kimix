...
* Retro button hover is one shared interaction language, not a component-by-component exception. Control, navigation-action, navigation-list, menu-trigger, context-bar, and standalone utility-button roles consume the same Retro hover border/background/shadow tokens. Never use a global native `button:hover` ring as fallback: interactive rows may contain nested button hit targets, and only the outer visual surface may own the hover boundary. New standalone buttons must enroll in a semantic button role explicitly.
