...
* Modern message surfaces use an explicit two-level hierarchy without becoming a new color theme. Neutral message metadata and Assistant inline-code shadows share `--kimix-modern-message-meta-background`, derived from the active palette's `--surface-hover` and `--surface-elevated`. User prompts derive again from that shared surface toward `--surface-elevated`, so they are always quieter than metadata/code regardless of palette. All three stay borderless and shadowless.
