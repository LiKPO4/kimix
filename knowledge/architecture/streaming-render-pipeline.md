...
timestamp: "2026-09-09T23:59:00+08:00"
---

# Streaming Render Pipeline

## Structured user media never becomes visible placeholder text
...
Automatic thinking translation never changes `TimelineEvent.thinking`, `thinkingParts`, active-turn offsets, or persisted official history. A visible thinking leaf observes the already-coalesced draft or settled `ThinkingBlock` and writes only to an in-memory `thinkingTranslationStore`, keyed by draft/block identity. Live sources wait for the configured 1–5 second coalescing window, submit only newly completed sentence/line chunks (bounded to 4,000 characters for Azure and 900 characters for the local Marian model), and keep at most one request in flight per key with a global concurrency ceiling. A settled block flushes its remaining tail immediately and adopts a compatible translated draft prefix only when the provider also matches, before translating the remaining source tail. Source or provider changes increment a request version and reset the derived prefix so stale or cross-provider responses cannot overwrite a newer selection; unmounting the last visible consumer cancels queued work, which prevents hidden/background sessions from spending translation quota. The full session/draft/block key remains the in-memory identity, but the optional IPC correlation `requestId` must use a bounded fingerprint plus version/offset/time metadata: official snapshot scopes can exceed the IPC schema's 200-character limit before any provider runs. Transient provider failures use bounded retry backoff, while completed inactive entries are retained only in a 160-entry LRU so long-running app sessions do not accumulate an unbounded translation cache.

The translation result is presentation data: until a chunk returns, the original tail remains visible; failures fall back to the complete original without affecting answer generation. Mostly-Chinese chunks bypass the provider, fenced and inline code are protected by placeholders, and translated-only versus bilingual rendering is chosen at the leaf. Renderer settings persist one mutually exclusive provider (`off`, `local`, or `azure`), interval, and display mode. Azure credentials and HTTP calls belong to the Electron main process, with the key stored separately through `safeStorage`. The local `Xenova/opus-mt-en-zh` q8 model is downloaded on demand into the Electron user-data directory (about 120 MB) and executed in a dedicated `utilityProcess`; model loading or ONNX inference must never run on the main process or Renderer thread. A ready marker distinguishes a complete cache, download progress is pushed over a narrow IPC event, and recursive deletion is allowed only for the validated model child directory. Future providers must preserve this scheduler/store and process-isolation boundary instead of attaching work to `thinking.delta` or rebuilding the parent `MessageBubble` on each tick.
...
- **Invariant S (derived change ownership outranks replay timestamps)**: `restoreLateHistoricalChangePlacement` may use timestamps to move an identity-less late historical `change_summary` and its sibling diff before a later user boundary, but it must first honor stable source ownership. If the summary shares an explicit `agentTurnId` / `roomMessageId` with the later user, or its `:change-summary` / `:diff` source id matches a `tool_call.toolCallId` physically located after that user boundary, the card belongs to the current turn even when replay supplied an older timestamp. Timestamp-only repair previously moved a current Write result into an already completed previous turn (v2.21.98); source tool identity is the stronger invariant.
- **Invariant T (canonical reconcile must re-attach renderer-local user media before veto gates)**: wire-derived canonical history never carries renderer-local image bytes (`dataUrl` pasted/dropped in the composer), so any session whose user ever attached an image has `canonicalImageCount < localImageCount` permanently. Feeding such a raw pair into `shouldReplaceWithCanonicalKimiHistory` always trips the `user-image-regression` veto — even when canonical is strictly richer in body, thinking, and process events — so a locally truncated timeline (e.g. renderer restarted mid-turn and missed the final body) can never self-heal through the repair/startup/settle reconcile paths (observed v2.21.191, session_493c0ce1 turn 24: local cut at the intermediate sentence, canonical held the 1273-char final body, verdict=false on the image gate alone). Every canonical-reconcile call site must first wrap canonical events with `preserveLocalUserMediaInCanonicalHistory(localEvents, canonical)` — the same antidote `kimiCodeSnapshotReplay` and `undoHistory` already used — before the circuit-breaker fingerprint, `reconcileAgentCanonicalHistory`, and `shouldReplaceWithCanonicalKimiHistory`. The preserved image count also shifts the circuit fingerprint versus any pre-fix rejection, reopening one retry automatically. Covered paths (v2.21.192): `repairKimiCodeHistoryBodies`, `recoverCollaborationRoomAtStartup` consumption point, startup hydration owner branch, and `reconcileFromHistorySnapshot` (terminal-tail / running-sample).
- **Invariant U (per-frame speed lookups must backfill the summary frame that is actually displayed)**: the vendored SDK emits `usage.record` (turn-scoped) first and then `agent.status.updated` carrying the cumulative `currentTurn` usage but no `usageScope`, while `filterStatusUpdates` turn_end mode displays the LAST metric frame of the turn — i.e. the scope-less summary frame. Any per-event-id metric lookup (`computeTurnUsageSpeeds`) keyed only on `usageScope==="turn"` frames therefore always misses the displayed pill and the rate never renders (observed v2.21.198: pill showed 模型/输入/输出 but no 速率). The map must backfill the latest computed turn speed onto subsequent main-agent status frames of the same turn segment, resetting at each user/main-tool boundary. The backfilled value must be the CUMULATIVE turn rate (turn output ÷ summed generation windows, same as `getSessionOutputStats.currentTurnSpeed`), never a single-step rate: the pill's 输出 shows cumulative turn tokens, and pairing it with a tiny final wrap-up call's single-step rate reads as an absurdly slow turn (v2.21.199 follow-up: 556 tokens shown next to 0.6 t/s).
  Window boundaries must ALSO include completed main-agent `tool_call` completion (`timestamp + durationMs`): on the SDK route `tool.result` is absorbed into the linked `tool_call` (mergeEvents records `durationMs`), so no standalone `tool_result` exists and every usage window otherwise starts at the user_message, stacking cumulatively (v2.21.199 second follow-up: 8.18k output shown at 2.4 t/s from a 5.6-minute turn).
- **Invariant V (nothing quadratic or full-history may run on the flush path)**: scroll jank during a running turn on a large session was NOT missing list virtualization — measured via CDP (lag-triggered CPU profile + devtools.timeline wheel-scroll trace, `scripts/cdp-scroll-trace.mjs`) the renderer was ~90% idle EXCEPT recurring 150–300ms main-thread tasks from two superlinear hot spots that re-ran on every stream flush: `filterStatusUpdates` did `findLastIndex`+`slice`+`filter` of the whole turn segment PER status event (O(statuses × events); 168ms per call on a 7.6k-event session — now a single O(n) pass grouping by (turn segment, lane), 1.63ms), and the Sidebar's per-row `isSessionSidebarBusy`→`hasActiveTimelineWork` rescanned every session's full events array whenever streaming replaced the `sessions` array reference (now WeakMap-cached by events-array identity with exact stale-window expiry semantics), plus `sessionIdentitySet` Set allocations in the O(n²) `dedupeSidebarSessions` pair loop (now WeakMap-cached by Session object identity). Past cadence/memo fixes only reduced how often this work ran; any per-flush work whose cost grows with total session size will reintroduce the same jank — new flush-path code must be O(new events), never O(history). Layout/paint are NOT the bottleneck (13ms Layout / 123ms Paint over a 14s scroll trace at 240Hz).
- **Invariant W (no O(N × total-length) re-normalization anywhere on the event or render path, v2.21.206/210/211)**: the same quadratic-in-disguise pattern kept resurfacing in different costumes — any merge/accumulate loop that re-runs `normalizeThinkingWhitespace`/regex-collapse/`includes`/suffix-prefix KMP over the ENTIRE accumulated text per incoming item is O(items × total length) and produces multi-second freezes on the 26.5-万-char / 417-event / 10-subagent session (recorded: `buildThinkingBlocks` 1095ms → incremental normalized view + tail-window KMP, v2.21.206; `mergeAssistantThinkingText` reduce in `mergeAssistantProcessEvents` ~3002ms → `mergeAssistantThinkingTextSequence`/`mergeAssistantThinkingTextStep` with per-draft `collapseForOverlap` view, v2.21.210/211; `deduplicateTimelineEvents` `assistantBodyKey` re-normalizing 26.5-万-char thinking per event per `mergeEvents` flush → WeakMap identity cache, v2.21.211). Rule: accumulation loops must carry an incremental normalized view (whitespace-run boundary merge, `appendCollapsedNormalized`) and overlap checks must use tail windows no longer than the incoming normalized length; identity-keyed work must be WeakMap-cached so repeated full-array passes degrade to O(1) map ops. When adding any merge/dedup/key function over assistant text, ask: does its cost grow with the ACCUMULATED size per call, and is it called per flush? If yes, it will freeze the heavy session again.
...
Assistant source soft breaks are preserved structurally instead: the shared
remark transform converts newline characters only inside mdast `text` nodes to
`break` nodes. This keeps poetry and line-oriented answers consistent between
the streaming plain/rich and settled paths without touching fenced/inline code,
tables, list structure, or paragraph boundaries.
