...
- **Invariant O (trailing text streams as the candidate final body)**: while a turn is active, the trailing text segment — the last text block with no tool/subagent/approval/question after it — streams live in the body area (formal committed text plus the uncommitted draft tail, prefix-safe appended). A following thinking phase does not demote it (avoid think/text alternation flicker); a real process boundary does, and the segment then renders in the process timeline through the existing skip logic. Withholding the final segment until settle (`computeFinalTextBlockContent` returning "" while streaming) plus the timeline skipping the trailing text made the whole answer invisible while the model composed it and appear at once at turn end.
