...
* The local-translation runtime (`@huggingface/transformers` + `onnxruntime-node`, ~98 MB unpacked) is **not packaged** since v2.21.177 — it downloads on demand into `userData/thinking-translation-runtime` (see `electron/localThinkingRuntimeManifest.ts` / `localThinkingRuntimeDownloader.ts`). `sharp` is replaced by a local stub (transformers imports it statically but translation never calls it; the packaged app never shipped sharp natives, so pre-v2.21.177 packaged local translation was latently broken). If a future image pipeline needs real sharp, add `@img/sharp-<platform>-<arch>` to the manifest instead of restoring the dependency. The worker must load the ESM entry (`dist/transformers.node.mjs`); the CJS entry hard-fails without sharp natives.
