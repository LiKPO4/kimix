...
timestamp: "2026-07-02T10:05:00+08:00"
---
...
* Startup and session switching use bottom-first rendering. A restored official session marked `isLoading` must not expose its persisted tail as authoritative because the official history can contain newer turns; Kimix shows an explicit bottom-aligned synchronization state until the first canonical history load completes, then renders the eager tail once. Server snapshot replay must propagate each message's official `created_at` to every derived frame and must never treat protocol sequence numbers as wall-clock timestamps. The eager tail uses a bottom-origin scroll coordinate system, so restored message growth extends upward while `scrollTop=0` remains the visual bottom. Background history hydration must not automatically expand the tail. Older visible-range items are prepended only after explicit upward-scroll intent or the earlier-history action; expansion returns to normal scroll coordinates and the added-height delta compensates `scrollTop` so the current tail remains stationary.
