...
description: Pinned facts about the official declarative subagent model pool ([secondary_model] config), its validation rules, and Kimix integration caveats through 0.41.0.
tags: [kimi-code, secondary-model, subagent, config, upstream]
timestamp: "2026-09-07T16:25:00+08:00"
---
...
- Through 0.39.x the pool was gated by the `secondary-model` experiment (`KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL=1` or the master flag). Kimi Code 0.40 enables this capability by default and 0.42.0 removes the experiment flag entirely (pool always-on); Kimix retains its explicit managed-process flag injection as a no-op for compatibility with older external/fallback runtimes. The v1 engine ignores the keys at runtime but round-trips them in config.
...
- The vendored SDK first moved to the 0.36.0 tag in v2.21.68 (`b6144f94`, node-sdk 0.17.0), making the pool live. It advanced to 0.38.0 / SDK 0.19.1 in v2.21.101, 0.39.0 / SDK 0.19.2 in v2.21.126 (`52e8d19d`), official 0.40.1 / SDK 0.20.0 in v2.21.163 (`0d45dddc`), and official 0.41.0 / SDK 0.20.0 in v2.21.178 (`95478e8c`); the pool remains active, with the 0.39 deletion-policy difference documented rather than hidden by bundle patches.
...
- [Release @moonshot-ai/kimi-code@0.40.1](https://github.com/MoonshotAI/kimi-code/releases/tag/%40moonshot-ai%2Fkimi-code%400.40.1)
- [Release @moonshot-ai/kimi-code@0.41.0](https://github.com/MoonshotAI/kimi-code/releases/tag/%40moonshot-ai%2Fkimi-code%400.41.0)
