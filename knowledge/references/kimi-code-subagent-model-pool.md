...
- Through 0.39.x the pool was gated by the `secondary-model` experiment (`KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL=1` or the master flag). Kimi Code 0.40 enables this capability by default; Kimix retains its explicit managed-process flag for compatibility with older external/fallback runtimes. The v1 engine ignores the keys at runtime but round-trips them in config.
